package sanliy.spider.novel.excelkt
...
abstract class ExcelElement {
     ...
     */
    fun createFont(init: XSSFFont.() -> Unit = { }): XSSFFont =
        xssfWorkbook.createFont().apply(init)
}
    ...
    init {
        xssfRow.createCell(index).run {
            ...
            when (content) {
                ...
                is LocalDate -> setCellValue(
                    Date.from(
                        content.atStartOfDay(ZoneId.systemDefault()).toInstant()
                    )
                )

                is LocalDateTime -> setCellValue(
                    Date.from(
                        content.atZone(ZoneId.systemDefault()).toInstant()
                    )
                )
